...
func TestUndumpWithSimpleFunction(t *testing.T) {
	// Create a more complex bytecode chunk with a simple function
	data := []byte{
		// Header
		// LUA_SIGNATURE (4 bytes) - "x1bLua" as bytes
		'x', '1', 'b', 'L',
		...
		// Code (1 instruction)
		0x01, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, // instruction 0
		// Constants (1 constant: nil)
		0x01, 0x00, 0x00, 0x00,
		TAG_NIL,
		// Upvalues (empty)
		0x00, 0x00, 0x00, 0x00,
	...
	// Verify the prototype structure
	if proto.LineDefined != 1 {
		t.Errorf("Expected LineDefined = 1, got %v", proto.LineDefined)
	}
	if proto.LastLineDefined != 2 {
		t.Errorf("Expected LastLineDefined = 2, got %v", proto.LastLineDefined)
	}
	if proto.NumParams != 1 {
		t.Errorf("Expected NumParams = 1, got %v", proto.NumParams)
	}
	if proto.MaxStackSize != 2 {
		t.Errorf("Expected MaxStackSize = 2, got %v", proto.MaxStackSize)
	}
	if len(proto.Code) != 1 {
		t.Errorf("Expected 1 instruction, got %v", len(proto.Code))
	}
	if len(proto.Constants) != 1 {
		t.Errorf("Expected 1 constant, got %v", len(proto.Constants))
	}
	if proto.Constants[0] != nil {
		t.Errorf("Expected nil constant, got %v", proto.Constants[0])
	}
}
